...
namespace Dungeons
{
	class DungeonRecord
	{
		...
		// 	public bool[] explored_rooms = null;

		//	public bool fIncluded = false;

		public DungeonRecord(int number)
		{
			fNumber = number;
			fCompleted = false;
			fOpenTime = DateTime.Now.Ticks;
		...
		{
			long span = fTreasureTime - DateTime.Now.Ticks;
			if (span > 0)
				return false;
			return true;
		}
		public void setClosed(int minutes)
		{
			...
			fOpenTime = DateTime.Now.Ticks + wait;
			Utility.Trace("DungeonRecord.setClosed - treasure in " + 6 * wait + "ms");
			fTreasureTime = DateTime.Now.Ticks + 6 * wait;
