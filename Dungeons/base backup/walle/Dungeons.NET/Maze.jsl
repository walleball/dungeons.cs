...
public class Maze
{
// 	public boolean fExplored = false;
	private Room[]	fRooms;
...
//	private Room	fHermit;
	...
	public boolean	fPlayerInWeb;
	static private boolean debug = false; // debug

	private static boolean[] sExploredRooms = null; 
...
//		fHermit = null;
	...
	public void setExplored(Room room)
	{
		if (!room.isExit())
		{
// 			boolean wasExplored = sExploredRooms[room.getRoomNumber() - 1];
...
// 			}
	...
	public boolean isExplored(Room room)
	{
	...
	public void initialize()
	{
		...
		{
			sExploredRooms = new boolean[GameEngine.kMaxNumRooms];
		...
		{
			int rnd = GameEngine.instance.rand.range(0, 100);
		...
		int iPortcullis = 0;
		Sequence seq = new Sequence(1, this.getNumRooms());
		while (seq.count() > 0)
		{
			no = seq.remove();
...
// 		}
	...
	public void distributeSwords(int count)
	{
        if (count > 0)
        {
            ...
            fContent += "Sword";
            if (count > 1)
                fContent += "*" + Integer.toString(count);
        }
...
//		return fLair;
//	}
	public Room getRandomLair(boolean force_create)
	{
		// Find room without door and only one entrance
		int passages = 1;
		boolean allow_door = false;
		while (true)
		{
			Sequence seq = new Sequence(2, this.getNumRooms());
...
//		return this.getRoom(1);
	...
	public boolean isWizard(Room room)
	{
		if (room == fWizard)
			return true;
		return false;
	}
	public boolean isJanitor(Room room)
	{
		if (room == fJanitor)
			return true;
		return false;
	...
	public void process()
	{
		...
		//
		Room room = fWizard;
		while (room == fWizard && fWizard != null)
		{
			int no = GameEngine.rand.range(1, this.getNumRooms());
			room = this.getRoom(no);
			if (room.hasMuck())
			{
				Utility.Trace("The wizard disposed of muck in cavern " + room.getRoomNumber());
				room.removeMuck();
			}
			break;
		}
		...
		if (fJanitor != null)
		{
			...
			else
			{
				...
				for (int i = 0 ; i < num ; i++)
				{
					...
					{
						room = tmp;
						Utility.Trace("The janitor smells muck in cavern " + room.getRoomNumber());
						break;
					}
                    if (tmp.fPit)
					{
						room = tmp;
						Utility.Trace("The janitor feels a draft from cavern " + room.getRoomNumber());
						break;
					}
                    if (tmp.fWeb)
					{
						room = tmp;
						Utility.Trace("The janitor sees a web in cavern " + room.getRoomNumber());
						break;
					}
...
//				}
                if (fJanitor.hasDoor() && fJanitor.fDoor == room)
				{
					if (fJanitor.isDoorLocked())
					{
						Utility.Trace("The janitor unlocks the door between cavern " + fJanitor.getRoomNumber() + " and " + room.getRoomNumber());
						fJanitor.unlockDoor();
						room.unlockDoor();
					...
					{
						Utility.Trace("The janitor opens the door between cavern " + fJanitor.getRoomNumber() + " and " + room.getRoomNumber());
						fJanitor.openDoor();
						room.openDoor();
					}
				}
				if (fJanitor.isBlockedByPortcullis(room))
				{
					Utility.Trace("The janitor raises the portcullis between cavern " + fJanitor.getRoomNumber() + " and " + room.getRoomNumber());
					fJanitor.openPortcullis(room);
					room.openPortcullis(fJanitor);
				}
				Utility.Trace("The janitor moves to cavern " + room.getRoomNumber());
				fJanitor = room;
			}
		}
		fPlayerInWeb = false;
	...
	public static final int USE_NO_DOORS = 0;
	public static final int USE_OPEN_DOORS = 1;
	public static final int USE_CLOSED_DOORS = 2;
	public static final int USE_LOCKED_DOORS = 3;
	public static final int USE_PORTCULLIS = 4;
	public static final int USE_EVERYTHING = 5;
...
// 		Utility.Assert(player == null || useDoors != USE_LOCKED_DOORS, "player == null || useDoors != USE_LOCKED_DOORS");
		...
		int[] path = new int[fNumRooms + 1];
		for (int i = 0 ; i <= fNumRooms ; i++)
			path[i] = 999;
		path[fromRoom.getRoomNumber()] = 1000 + fromRoom.getRoomNumber();
					...
					for (int iPassage = 1 ; iPassage <= room.getNumPassages() ; iPassage++)
					{
						if (room.isPassable(iPassage) || useDoors >= USE_PORTCULLIS)
						{
							...
							if (player == null || player.hasExplored(room) || player.hasExplored(tmpRoom))
								if (path[tmpRoom.getRoomNumber()] == 999)
									path[tmpRoom.getRoomNumber()] = -iRoom;
						}
					}
					if (room.hasDoor())
					{
						boolean allowDoor = true;
						if (player != null)
						{
...
//							Utility.Assert(player == null || player.hasVisited(room) || player.hasVisited(door), "Maze.findRoute - player == null || player.hasVisited(room) || player.hasVisited(door)");
							if (player == null || player.hasExplored(room) || player.hasExplored(door))
								if (path[door.getRoomNumber()] == 999 && !door.isExit())
									path[door.getRoomNumber()] = -iRoom;
						}
					}
					exhausted = false;
		...
		if (path[toRoom.getRoomNumber()] == 999)
		{
