...
namespace Dungeons
{
	class StaffContainer
	{
		...
		public String getDescription()
		{
			...
			//             str += Convert.toString(staff.getType()) + " ";
			//         }
			//         return str;

			int[] types = new int[Staff.MAX_STAFF + 1];
			for (int i = 0; i < Staff.MAX_STAFF; i++)
			{
				types[i] = 0;
			}
			for (int i = 0; i < this.getNumStaffs(); i++)
			...
			String str = "";
			if (types[Staff.PALLADIUM] > 0)
			{
				str += Convert.ToString(types[Staff.PALLADIUM]) + " palladium ";
			}
			if (types[Staff.PLATINUM] > 0)
			{
				str += Convert.ToString(types[Staff.PLATINUM]) + " platinum ";
			}
			if (types[Staff.GOLD] > 0)
			{
				str += Convert.ToString(types[Staff.GOLD]) + " gold ";
			}
			if (types[Staff.SILVER] > 0)
			{
				str += Convert.ToString(types[Staff.SILVER]) + " silver ";
			}
			if (types[Staff.BRONZE] > 0)
			{
				str += Convert.ToString(types[Staff.BRONZE]) + " bronze ";
			}
			if (types[Staff.WOOD] > 0)
			{
				str += Convert.ToString(types[Staff.WOOD]) + " wood ";
			}
			return str;
		}
		...
		public String getVerboseDescription()
		{
			...
			if (types[Staff.PALLADIUM] > 0)
			{
				str += Utility.numToStr(types[Staff.PALLADIUM], "A Palladium Staff", "Palladium Staffs") + "\n";
			}
			if (types[Staff.PLATINUM] > 0)
			{
